...
from . import sms, utilities, sender, account 
#import sms, utilities, sender, account 
import logging
...
def example_02_change_pass(mobile, old_pass, new_pass):
    account = account.MobilyAccount(utilities.MobilyApiAuth(mobile, old_pass))
    account.change_password(new_pass)


def example_03_forgot_pass_email(mobile, password):
    account = account.MobilyAccount(utilities.MobilyApiAuth(mobile, password))
...
def example_06_send_sms(mobile, password, recipient_mobile):
    sms = sms.MobilySMS(utilities.MobilyApiAuth(mobile, password))
...
def example_09_send_formatted_sms(mobile, password, recipient_one, recipient_two):
    auth = utilities.MobilyApiAuth(mobile, password)
    msg = 'Hi (1), your subscription will end on (2).'
    sms = sms.MobilyFormattedSMS(auth, [recipient_one, recipient_two], 'PYTHON', msg)
...
def example_11_add_alpha_sender(mobile, password, alpha_sender):
    sender = sender.MobilySender(utilities.MobilyApiAuth(mobile, password))
...
def example_13_add_mobile_sender(mobile, password, mobile_sender):
    ...
    except utilities.MobilyApiError as e:
...
def tms_action_send_post_sms():
    auth = utilities.MobilyApiAuth('smb', 'asQW123')
    
    new_receipt_msg = "Hello Nabil"
    print(new_receipt_msg)
    try:
        snd = sender.MobilySMS(auth, ['0535906170'], 'SMB', new_receipt_msg)
    except Exception as e:
        print("Error After sending sms: {0}".format(e))
        return
    try:
        response = snd.send()
        print("Success Sending SMS: {0}".format(response))
    except Exception as e:
        print("Error Sending SMS: {0}".format(e))

#tms_action_send_post_sms();

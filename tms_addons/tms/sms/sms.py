...
class MobilySMS(object):
    def __init__(self, auth, numbers=None, sender='', msg='', delete_key=None, msg_id=None,
        ...
        self.request_handler = utilities.MobilyApiJsonRequestHandler(self.auth)
    ...
    def can_send():
        ...
        request_handler = utilities.MobilyApiJsonRequestHandler()
        request_handler.set_api_method('sendStatus')
        try:
            response = request_handler.handle()
        except utilities.MobilyApiError:
            return False
        else:
    ...
    def delete(self):
        ...
        if self.delete_key is None:
            return
        request_handler = utilities.MobilyApiJsonRequestHandler(self.auth)
...
class MobilyFormattedSMS(MobilySMS):
    ...
    def _prepare_to_send(self):
        ...
        self.request_handler.add_parameter('msg', utilities.MobilyApiUnicodeConverter.convert(self.msg))
        self.request_handler.add_parameter('msgKey', utilities.MobilyApiUnicodeConverter.convert(self.generate_msg_key()))

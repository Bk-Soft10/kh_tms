    ...
    def get_item_data(self):
        ...
        if self.from_date and self.date_end:
            ...
            for vals in res:
                ...
                column += 1
            worksheet.write(row, column, 'Bank', cell_text_format)
            column += 1
            worksheet.write(row, column, 'Account Number', cell_text_format)
            column += 1
            worksheet.write(row, column, 'IBAN', cell_text_format)
            row = row + 1
            ...
            if self.filter == 'all':
                ...
                payslip_ids=self.env['hr.payslip'].search([('date_from','=',self.from_date),('date_to','=',self.date_end),('payslip_run_id','=',self.batch_id.id)])
            if payslip_ids:
                for payslip in payslip_ids:
                    ...
                    column = column + 1
                    worksheet.write(row, column, payslip.employee_id.bank_account_id and payslip.employee_id.bank_account_id.bank_id and payslip.employee_id.bank_account_id.bank_id.name or '', cell_text_format_new)
                    column = column + 1
                    worksheet.write(row, column, payslip.employee_id.bank_account_id and payslip.employee_id.bank_account_id.acc_number or '', cell_text_format_new)
                    column = column + 1
                    worksheet.write(row, column, payslip.employee_id.bank_account_id and payslip.employee_id.bank_account_id.iban or '', cell_text_format_new)
                    row += 1
        ...
        excel_id = self.env['payroll.report.excel'].create({"name": file_name,
                                                          "file_download": out})
        _logger.info('UUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUU')
...
class payroll_report_excel(models.TransientModel):
    ...
    file_download = fields.Binary('Download payroll', readonly=True, attachment=False)

    ...
    def action_download_excel(self):
        ...
        if not excel_sheet or not excel_filename:
            return {
                ...
                'params': {
                    'message': 'No Excel template found in settings.',
                    'type': 'warning',
                    'sticky': False,
                }
            }
        ...
        return {
            ...
            'url': '/web/binary/download_excel_template',
            'target': 'self',
        }
                ...
                for record in xl_order.iter_rows(min_row=2, max_row=None,
                    ...
                    if line[0] and line[1] and line[3]:
                        partner = self.env['res.partner'].search(
                            [('name', '=', line[3])])
                        if line[2]:
                            if isinstance(line[2], datetime):
                                date_obj = line[2].strftime("%Y-%m-%d")
                            elif isinstance(line[2], str):
                                date_obj = datetime.strptime(line[2], "%Y-%m-%d")
                            else:
                                date_obj = line[2]  # Assuming line[2] is already a date
                        else:
